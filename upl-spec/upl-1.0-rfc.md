...
| `ofields`      | Conditional | `object`, `object_shape`, and `list`/`option_single`/`option_multi` whose `etype` is the inline `object` | Map of object field definitions (recursive). Required on `object_shape`; on `object` either `ofields` (inline) or `type: <object_shape_name>` (§3.4.2) must be present, but not both. On a `list`/`option_*` it declares the element shape of an inline `etype: object` (§3.3, §4.1.1) — a by-name `etype: <object_shape>` reference takes its shape from the referenced object_shape instead, and must not also declare `ofields`. |
...
| `list`           | List of free entered values                     | **Yes**                              | Only with an inline `etype: object` | No               |
...
| `option_single`  | Single choice from a list of options           | No — optional, defaults to `string`  | Only with an inline `etype: object` | **Yes** (≥ 2)    |
| `option_multi`   | Multiple choices from a list of options        | **Yes**                              | Only with an inline `etype: object` | **Yes** (≥ 2)    |
...
4. **Validate field types** — for each variable, ensure `def` and `opts` values match the
   ...
   For `option_single`/`option_multi`, also ensure the `def` is one of the declared `opts`
   (every element, for `option_multi`) — a `def` that is not an offered option is a parse
   error (§3.3).
...
- Rejects `def` values whose kind does not match the declared `type`/`etype` (§3.3) as
  parse errors, and `option_single`/`option_multi` `def` values that are not among the
  declared `opts` (§3.3).
- Rejects tab characters in the indentation of the metadata section (§2), whatever column
  the tab-indented line would otherwise land on.
