...
* Using an UPL prompt you can interactively build a prompt by entering the value of each variable depending on your use case.
* You can also build a prompt from an UPL prompt by providing a json with the variables filled in
* UPL prompts can be autogenerated using an llm by installing [upl-prompt-skills](https://github.com/DavidValin/upl-prompt-skills) in your agent (ex. opencode, claude...)
...
7. UPL `repository client`, to publish/download upl prompts to/from an upl repository

All operations are integrated in a single cli command called `upl`.

### Quickview

* Specification: [`upl-spec/upl-1.0-rfc.md`](upl-spec/upl-1.0-rfc.md) (`make doc` renders it to `upl-spec/upl-1.0-rfc.pdf`)
...
Options:
  ...
  --user, -u        Provide a username non-interactively (login, register_user).
  --password, -p    Provide a password non-interactively (login, register_user).
  --visibility      Visibility for `push`: public|private (default: private).
  --tls             Use TLS when connecting to the repository (set-rep).
  --gpg-key, -g     Path to a GPG public key file or key id (register_user).
